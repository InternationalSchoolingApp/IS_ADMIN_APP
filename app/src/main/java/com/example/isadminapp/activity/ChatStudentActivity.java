...
import com.example.isadminapp.databinding.ActivityChatStudentBinding;
import com.example.isadminapp.fcmApi.FcmApiClient;
import com.example.isadminapp.fcmApi.FcmApiInterface;
...
public class ChatStudentActivity extends AppCompatActivity {

    private ActivityChatStudentBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityChatStudentBinding.inflate(getLayoutInflater());
        ...
        if (extra != null) {
            ...
            recieverEmail = extra.getString("email").toLowerCase();
    ...
    private void sendMessage() {
        ...
        if (conversionId != null) {
            ...
            conversion.put("studentName", recieverName);
            conversion.put("studentEmail", recieverEmail);
    ...
    private void listenAvailabilityOfReceiver() {
        database.collection(Constants.FIREBASE_USER_DB).document(""+recieverEmail).addSnapshotListener(ChatStudentActivity.this, ((value, error) -> {
            if (error != null) {
                return;
            }
            if (value != null) {
    ...
    private void checkForConversionRemotely(String senderId) {
        database.collection("ADMIN_STUDENT_CONVERSATION").whereEqualTo(Constants.KEY_SENDER_ID, senderId).whereEqualTo(Constants.KEY_RECIEVER_ID, recieverEmail).get().addOnCompleteListener(conversionCompleteListener);
    }
    ...
    private void addConversion(HashMap<String, Object> conversion) {
        ...
        conversation.document(""+recieverEmail+" - "+preferenceManager.getString(Constants.USER_EMAIL).toLowerCase()).set(conversion).addOnSuccessListener(documentReference -> conversionId = (""+preferenceManager.getString(Constants.USER_EMAIL).toLowerCase()+" - "+recieverEmail)).addOnFailureListener(exception->{
        });
    ...
    private void updateConversion(String message) {
        ...
        documentReference.update(Constants.KEY_LAST_MESSAGE, message, Constants.KEY_TIME_STAMP, new Date(), Constants.KEY_SENDER_ID, senderId,
                ...
                "studentEmail", recieverEmail,
                "studentName", recieverName,
